...
테스트 결과 평균적으로 1분당 눈을 20번, 2초 감는다. 따라서 이 둘을 같은 비율로 나눠서 점수에 반영했다.
각각의 코드는 다음과 같다
```bash
...
```bash
    ...
    return (A + B) / (2.0 * C)
```
눈을 감았는지를 판별하는 EAR(Eye Aspect Ratio)이다.
눈 주위의 6개의 점을 찍어 상하길이 평균을 좌우길이로 나눠 판정한다.
또한 이후에 왼쪽눈과 오른쪽 눈을 평균을 내 0.2보다 작으면 눈을 감았다고 판정한다.
이 프로젝트에서는 0.2를 기준으로 잡았지만 사람마다 눈의 모양이 다르므로 기준을 변경하면 된다.
...
if ear < 0.2:
    if eye_closed_start is None:
        ...
        if not alarm_playing:
            ...
            alarm_playing = True
```
EAR이 0.2보다 낮은 상태로 3초간 유지되면 알람을 재생한다.
알람이 중복되게 울리지 않게 했다.
...
```
카메라가 매우 빠르게 프레임을 처리하기 때문에 1분마다 한 번만 값을 저장 및 그래프 갱신하도록 next_minute_mark 기준을 사용했다.
저장되는 값은 다음과 같다
...
<img src="https://img.shields.io/badge/표시할이름-색상?style=for-the-badge&logo=python&logoColor=white">
...
특히 얼굴 인식 과정에서 68개의 얼굴 랜드마크 포인트를 활용하는 방식을 접하고 이를 기반으로 눈의 상태를 판단하는 로직을 구현한 경험이 인상 깊었다.
단순한 이미지 처리에서 나아가, 실제 사람의 행동을 감지하고 시각화하는 과정까지 구현해보며 컴퓨터 비전 기술이 실제로 어떻게 적용되는지 실감할 수 있었다.
